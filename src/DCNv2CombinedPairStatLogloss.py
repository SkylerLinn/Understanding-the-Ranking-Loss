...
import numpy as np
class DCNv2CombinedPairStatLogloss(BaseModel):
    def __init__(self, 
                 feature_map, 
                 model_id="DCNv2", 
                 gpu=-1,
                 ...
                 pos_weight=1.,
                 **kwargs):
        super(DCNv2CombinedPairStatLogloss, self).__init__(feature_map, 
                                    model_id=model_id, 
                                    gpu=gpu, 
    ...
    def compute_loss(self, return_dict, y_true):
        ...
        logging.info("Train BCE: {}".format(pointwise_loss))
    ...
    def evaluate_metrics(self, y_true, y_pred, metrics, group_id=None):
        ...
        if 'wlogloss' in metrics:
            sample_weight=np.where(y_true==1., self.pos_weight, 1.)
            ret_dict={'wlogloss':log_loss(y_true=y_true, y_pred=y_pred,sample_weight=sample_weight)}
        else:
            ret_dict = dict()
