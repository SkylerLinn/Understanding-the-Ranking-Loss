...
import numpy as np
class DCNv2CombinedListGradFirstEpoch(BaseModel):
    def __init__(self, 
                 feature_map, 
                 model_id="DCNv2", 
                 gpu=-1,
                 ...
                 pos_weight=1.,
                 **kwargs):
        super(DCNv2CombinedListGradFirstEpoch, self).__init__(feature_map, 
                                    model_id=model_id, 
                                    gpu=gpu, 
    ...
    def compute_loss(self, return_dict, y_true):
        ...
        exp_logit_1dim = torch.exp(logit_1dim)
        denorm = torch.sum(exp_logit_1dim[index_pos])*self.pos_weight + torch.sum(exp_logit_1dim[index_neg])
        listwise_loss = torch.mean(-torch.log(exp_logit_1dim[index_pos]/denorm)) # 外面本来有N+的数量，但是求均值又除以N+，抵消了
        ...
        return loss,index_pos,index_neg,logit
