...
if __name__ == '__main__':
    ...
    def compute_loss(y_pred, y_true):
        ...
        loss = model.loss_fn(y_pred, y_true, reduction='mean',weight=weight)
        return loss
            ...
            for batch_data in data_generator:
                for wb in tqdm(beta):
                    for wa in alpha:
                        ...
                        pointwise_loss = compute_loss(y_pred,y_true)
                        
                        alphas.append(wa)
                        betas.append(wb)

                        pointwise_losses.append(float(pointwise_loss))
                break
        
        alpha_grid, beta_grid = np.meshgrid(alpha, beta)
        np.save('./analysis_tools/loss_landscape/bce_loss.npy',np.array(pointwise_losses).reshape(alpha_grid.shape))
